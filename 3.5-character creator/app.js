...
function generateCharacter() {
  ...
  //Checks that only letters are input for names
  if(!onlyLetters(name)) {
...
function renderAttributesTable() {
  ...
  for(var i = 0; i < rolls.length; i++) {
    ...
    if(i > 0) {
      ...
      upDiv.innerHTML = '<img class="up-arrow" src="assets/arrow22.png" alt="Up"/>'; //Up arrow
    ...
    if(i < rolls.length - 1) {
      ...
      dwnDiv.innerHTML = '<img class="down-arrow" src="assets/arrow22.png" alt="Down"/>'; //Down arrow
      dwnDiv.addEventListener('click', handleDwnClick);
      rollDiv.appendChild(dwnDiv);
    }
    //Extra rolls that will be discarded if not utilised
    if(i ===  6) {
...
function handleSbmtDivClick() {
  ...
  //Sends you to the display page for the character
...
//Race values assigned to main values based on race selected
...
//Loop through myCharacter array and makes the sidebar clickable for accessable characters
...
//Init empty array to store elements named 'click'
var aTags = [];
function eventAdder (){
...
function handleSubmitClickSideBar(event) {
  ...
  //Storge character data in local storage with JSON
}
